...
const mockPosts = [
	{
		...
		date: "2026-08-01",
		categories: [
			{ title: "News", url: "/category/news", type: "Category" as const },
		],
	},
	{
		...
		date: "2026-08-02",
		categories: [
			{ title: "Updates", url: "/category/updates", type: "Category" as const },
		],
	},
	{
		url: "/blog/third-post",
...
export const WithInjectedComponent: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		expect(canvas.getByText("First Post")).toBeInTheDocument();
		expect(
			canvas.getByText("Subscribe to our newsletter!"),
		).toBeInTheDocument();
		expect(canvas.getByText("Second Post")).toBeInTheDocument();
	},
};
