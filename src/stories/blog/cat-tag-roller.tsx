  ...
  return (
    <div className="flex flex-wrap gap-2 font-mono text-sm">
      {combined.map((item, idx) => (
        <Link key={`${item.type}-${item.title}-${idx}`} to={item.url} className="no-underline hover:no-underline">
          <Badge variant={item.type === 'Tag' ? 'secondary' : 'default'} className="cursor-pointer">
