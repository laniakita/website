...
export function TableOfContentsDesktop(props: ToCMenuProps) {
	...
	return (
		<nav
			...
			className={`${tocInView ? "min-w-80 md:w-80 lg:w-96 lg:min-w-96" : "w-0 min-w-0"} sticky top-0 hidden max-h-dvh overflow-y-auto overflow-x-hidden border-border border-r bg-muted text-foreground shadow-xl md:block motion-safe:[transition:width_0.8s,min-width_0.8s,background-color_0.5s]`}
		>
			<div className='sticky top-0 z-10 flex min-h-16 w-full flex-row items-center justify-start px-6 text-foreground'>
...
export function TableOfContentsMobile(props: ToCMenuProps) {
	...
	return (
		<Collapsible
			...
			className={`sticky top-16 z-40 block size-full md:hidden ${inView ? "translate-y-0" : "-translate-y-16"} duration-300 motion-safe:transition-transform`}
		>
				...
				<div className={`absolute z-30 flex w-full flex-row items-center`}>
					...
					<div className='relative z-35 flex size-full h-12 flex-row items-center gap-4 px-6'>
						<CollapsibleTrigger
							...
							className={`z-40 -m-1.5 flex items-center whitespace-pre font-mono text-muted-foreground text-sm transition-colors hover:text-primary ${isOpen ? "text-primary" : ""}`}
						>
							<span
								className={`${isOpen ? "transform-[rotate(90deg)_translate3d(-0.1rem,-0.2ch,0px)]" : "transform-[translate3d(-0.1rem,0.0ch,0px)]"} icon-[ph--caret-right-bold] pointer-events-none mr-[0.5ch] w-[2ch] text-xl [transition:transform_0.3s]`}
							/>
							On this page
						</CollapsibleTrigger>

						<p className='z-40 flex min-w-0 flex-row items-center gap-[1ch] overflow-x-hidden whitespace-pre font-mono text-foreground text-sm'>
							<span className='icon-[ph--caret-double-right-bold] min-w-[2ch] text-muted-foreground text-xl' />
			...
			<AnimatePresence>
				{isOpen && (
					<motion.div
						...
						className='fixed top-28 left-0 z-20 h-[calc(100dvh-7rem)] w-full bg-black/20 backdrop-blur-sm dark:bg-black/40'
			...
			<CollapsibleContent
				className={`absolute top-12.5 right-0 left-0 z-30 h-(--disclosure-panel-height,0px) max-h-[calc(100dvh-12rem)] overflow-y-auto overflow-x-hidden border-border bg-background/95 shadow-xl backdrop-blur-md transition-[height] duration-300 ease-in-out ${isOpen ? "border-b" : "border-b-0"}`}
			>
