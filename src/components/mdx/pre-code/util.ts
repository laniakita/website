...
export const codeCollapser = (
	...
	if ((props.children as ReactElement).type === "code") {
		...
		if (
			typeof codeBlock.props.children !== "string" &&
			codeBlock.props.children?.[0]?.type === "div"
		) {
			if (
				"data-line-number" in
					(codeBlock.props.children[codeBlock.props.children.length - 1]
						.props as NumberedCodeBlockProps) &&
				parseInt(
					(
						codeBlock.props.children[codeBlock.props.children.length - 1]
							.props as NumberedCodeBlockProps
