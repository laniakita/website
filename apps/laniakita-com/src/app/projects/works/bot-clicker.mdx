...
descr: Bot Clicker is a toy satirical "game" I made for the landing page where you click on robots.
...
2. Bot Clicker needed somewhat high fidelity graphics (i.e. post-processing shaders) with a decent framerate to boot.
   Admitedly the second goal wasn't that challenging to achieve since many optimization implementations (instancing, LOD, etc.) do come OOTB with Three.js (it also didn't hurt that the example I was using as inspiration had solved a very similar set of problems). Still, there were some tiny hills left for me to meander over to get things running as well as they do.
