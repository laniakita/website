...
const getAbout = createServerFn().handler(async () => {
	...
	const description = descriptionHelper(await page?.data.getText("processed"), "/about");
	const RenderableInfo = await renderServerComponent(<GlobalMDXRenderer>{description ?? ""}</GlobalMDXRenderer>);
...
export const Route = createFileRoute("/(core)/")({
	loader: async () => {
		...
		const { RenderableInfo } = await getAbout();
		return { PostRoller: Renderable, meta: { categories, tags }, Info: RenderableInfo };
	},
...
function App() {
	const { PostRoller, meta, Info } = Route.useLoaderData();
	...
	return (
		...
		>
			<div className='m-auto flex size-full max-w-7xl flex-row gap-6 px-2 pt-4 md:p-10'>
				<main className='simple-color-trans m-auto flex flex-col justify-center gap-4 px-page-common pt-blog md:flex-row md:gap-6'>
					<SidebarInfo className='md:hidden' categories={meta.categories} tags={meta.tags} blogInfo={Info} />
					{PostRoller}
				</main>
				<div className='hidden md:flex md:w-full md:max-w-xs lg:max-w-sm'>
					<BlogSidebar categories={meta.categories} tags={meta.tags} info={Info} />
				</div>
			</div>
		</Suspense>
	);
