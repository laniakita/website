...
export const BlueskyEmbedCore = ({ postUrl }: { postUrl: string }) => {
	const [data, setData] = useState<AppBskyFeedGetPostThread.OutputSchema>();
	...
	if (data?.thread.$type === "app.bsky.feed.defs#threadViewPost") {
		const postData = data.thread as AppBskyFeedDefs.ThreadViewPost;
		...
		if (postData.post.embed?.$type === "app.bsky.embed.images#view" && "images" in postData.post.embed) {
			...
			return (
				<div className='not-prose flex w-full items-center justify-center -gap-2 rounded-2xl border border-ctp-surface0 bg-ctp-base p-4'>
						...
						<div className='flex flex-row items-center gap-2'>
							...
							<p className='flex flex-col -space-y-1'>
								...
								<a
									...
									className='text-ctp-subtext0 text-sm no-underline hover:text-ctp-subtext1'
								>
									@{handle}
								</a>
							</p>
