    ...
    <>
      ...
      >
        ...
        <div
          ...
          className={`${showMobileMenu ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'} fixed inset-x-0 top-12 bottom-0 z-20 size-full h-dvh w-full flex-col justify-start bg-black/40 [transition-timing-function:_cubic-bezier(0.4,0,0.2,1)] [perspective:_5px] motion-safe:[transition:_opacity_0.3s,]`}
        >
          <div
            aria-label='Table of contents'
            ref={dropToCRef}
            className={`${showMobileMenu ? '[transform:translate3d(0%,0%,0px)] opacity-100' : 'pointer-events-none [transform:translate3d(0%,-100%,-0.01rem)] opacity-0'} inset-x-0 top-28 bottom-0 z-20 max-h-[calc(100vh-7rem)] w-full overflow-auto rounded-b-2xl border-b border-ctp-pink bg-ctp-base/90 px-6 py-10 backdrop-blur-md [transition-timing-function:_cubic-bezier(0.4,0,0.2,1)] motion-safe:[transition:transform_0.8s,_opacity_0.5s,_background-color_0.8s] md:hidden dark:border-ctp-sky dark:bg-ctp-midnight/90`}
          >
            {shouldRun && (
              <Headings
                tree={readyHeadings}
      ...
      <nav className='sticky top-0 hidden h-dvh w-full max-w-sm min-w-72 items-center justify-start gap-10 overflow-y-auto bg-ctp-base/20 pb-10 text-slate-100 shadow-xl motion-safe:simple-color-trans md:flex md:flex-col dark:bg-ctp-base/20'>
