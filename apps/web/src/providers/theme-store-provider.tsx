...
export function DarkStoreProvider({
  children
}: DarkStoreProviderProps) {
  ...
  return (
    <DarkStoreContext.Provider value={darkStoreRef.current}>
      {children}
    </DarkStoreContext.Provider>
  )
}

export const useDarkStore = <T,>(
  selector: (store: DarkStore) => T,
): T => {
