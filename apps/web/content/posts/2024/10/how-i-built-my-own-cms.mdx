...
imageSrc: ../../../assets/images/non-oc/a-custom-cms.png
altText: "A parody, that features a main figure, with a Bun for a head and a blindfold featuring MDX across it. TypeScript, Drizzle, and Turso are background characters. A character with a WP for a head, represents a standard CMS, and is eating popcorn in the corner."
...
*A previous version of this article thought this would be a series of posts. I've since decided that it would be better to continue the long format after all. So, enjoy this updated article, now fully complete.*
...
Once the `finalArr` is generated from `batchFetchMDXPaths`, the posts can then be read into memory, and move on to *processing* via `batchFetchFrontMatter()`.

### Post Processing & Content Data Generation

With all the data in memory, it became possible to actually *do something* with it. That's where the pre-processing step (of the posts) came into play. Just like before, we'll start with the main function (`batchFetchFrontMatter()`) and work our way through it.
...
Then it gets to the heart of the function, the `matterProcessor`, which is, *"Ha, quite a doozy"* (we'll break it down next), and returns the result:
...
Then we have the actual function `comboImageProcessing()`, that leverage inner functions to both generates the `srcset` and dimension data, and performs the copying step to the public folder.
...
This function finds the absolute image path from the relative path given in the front matter, and then reads it into memory as an `arrayBuffer`, leveraging Bun's File I/O API. Then it transforms it from an array to simply a `Buffer`, which can be used as the input for `getPlaiceHolder()`. That function from `plaiceholder`, is what generates the blurry `srcset` and provides the image dimensions.
...
With that out of the way, the final step is to copy the image from the `content` folder, to the public folder, which Next.js can use to serve the images. The below function both performs that step, as well as generates the new relative path (to the public folder), which will be saved into the in-memory post object.
...
From there, we can zoom further back out to the parent function to notice we return the assembled *processed* object as the `res`, to generate the `metaArr` array, which is then sorted and returned according to the priority we defined earlier:
...
While the latter problem could've been solved by simply creating/importing those function types, this script was written at a point where I thought I could release this thing to the public as ready to use software. The biggest hurdle to that I later realized, was: *how was it supposed to generate the tables people wanted?* Again, I didn't solve that problem, because by that point, I was set on moving over to something that seemed less brittle, i.e., `contentlayer`.

As well, you'll notice I'm importing the main fetch function (`batchFetchMain()`), which both fetches the posts to read, and performs various processing steps. If you'll recall, many of those optimizations steps are admittedly quite *rigid*, or *inflexible*, which would create further problems if I ever did wind up generalizing this code based content management system.

Tech debt aside, you can see how my generic script, integrated with the configuration object defined in the next section.
...
With everything now in place, every time I ran `next build`, content would be scanned, processed, and then fresh content would be inserted into the database, while old content would remain the same. I was then able to retrieve it using the functions I wrote in the previous section.
...
The above is sorta complicated on first look, but the important thing to notice is that I'm taking the retrieved raw `mdx` string, and feeding into my `mdx-bundler` functions, which declares all the plugins I use for these posts.

Et voila! A CMS I built all by myself, with data flowing from a database, directly into this Next.js application, to generate these posts.
