...
Bypassing SST's `buildCommand`, like I did, _the rub_ is that you lose out on having resources passed down to the application's build environment. While the resources part can be solved with wrapping `turbo deploy` with `sst shell` i.e. `sst shell bun run turbo deploy`, the environmental variables that you can define in a config, apparently don't.
...
Given everything _just works_, like it should. Like it did before, **for months**, leading up to this. I'll admit to feeling a _little_ vexed.
